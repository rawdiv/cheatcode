...
app.get('/api/problems', async (req, res) => {
  try {
    ...
    // Check if user is admin (this is a simple check - you'd want more robust auth)
    // TEMPORARILY BYPASSED - DO NOT USE IN PRODUCTION
...
app.post('/api/problems', async (req, res) => {
  try {
    ...
    if (!title || !description || !difficulty || !examples || !solution) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    
    // Create a base slug if not provided
    let baseSlug = slug || title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    ...
    while (slugExists) {
      ...
      if (error || !data) {
        slugExists = false; // Slug doesn't exist, we can use it
      } else {
    ...
    // Insert problem with unique slug
    const { data, error } = await supabase
      .from('problems')
      .insert([{
        title,
    ...
    res.status(201).json({ success: true, problem: data[0] });
  } catch (error) {
...
app.post('/api/problems/bulk-delete', async (req, res) => {
  try {
    // Check if user is admin (TEMPORARILY BYPASSED)
    // if (!req.session.user || !req.session.user.email || !req.session.user.email.includes('admin')) {
    //   return res.status(403).json({ error: 'Unauthorized access' });
    ...
    res.status(200).json({ success: true, count: problemIds.length });
  } catch (error) {
...
app.post('/api/problems/bulk', async (req, res) => {
  try {
    ...
    // Track inserted problems and errors
    const results = {
      success: [],
      errors: []
    };

    // Insert problems one by one to handle constraints
    for (const problem of problems) {
      try {
        // Generate base slug
        let baseSlug = problem.slug || problem.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        ...
        // Prepare problem with unique slug
        ...
        // Store extra data in the description field as JSON inside HTML comments
        try {
          ...
          // Append the data to the description in a way that won't be visible to users
          processedProblem.description = `${processedProblem.description}\n\n<!-- PROBLEM_DATA: ${extraData} -->`;
        } catch (error) {
          console.error('Error stringifying problem data:', error);
        }
        
        // Insert the problem
        const { data, error } = await supabase
          .from('problems')
          .insert([processedProblem])
          .select();
        
        if (error) {
          results.errors.push({ problem: problem.title, error: error.message });
        } else {
          results.success.push(data[0]);
        }
      } catch (err) {
        results.errors.push({ problem: problem.title, error: err.message });
      }
    }
...
app.post('/api/verify-solution', async (req, res) => {
  try {
    ...
    // Extract test cases from either the test_cases table or the testCases JSONB field
    let testCases = [];
    if (problem.test_cases && Array.isArray(problem.test_cases)) {
      testCases = problem.test_cases;
    } else if (problem.testCases && Array.isArray(problem.testCases)) {
      testCases = problem.testCases;
    } else if (problem.examples && Array.isArray(problem.examples)) {
      // Fallback to using examples as test cases
    ...
    if (!testCases || testCases.length === 0) {
      // Create a default test case from the first example
    ...
    const sandbox = {
      console: {
        log: (...args) => {}, // Suppress console.log
        error: (...args) => {} // Suppress console.error
      },
      module: { exports: {} }
    };
    ...
    if (language === 'javascript') {
      // For JavaScript, we need to wrap the code in a function
      executableCode = `
        ${code}
    ...
    try {
      ...
      solutionFn = sandbox.module.exports.solution;
    } catch (error) {
      return res.status(400).json({
        status: 'Error',
        error: 'Failed to compile code: ' + error.message,
        testResults: [],
        time: '0 ms',
        memory: '0 MB'
      });
    ...
    if (!solutionFn) {
      return res.status(400).json({
        status: 'Error',
        error: 'No solution function found. Please implement a function named "solution" or the problem-specific function.',
        testResults: [],
        time: '0 ms',
        memory: '0 MB'
      });
      ...
      try {
        ...
        try {
          input = typeof testCase.input === 'string' ? 
            JSON.parse(testCase.input.replace(/'/g, '"')) : testCase.input;
          expected = typeof testCase.expected === 'string' ? 
            JSON.parse(testCase.expected.replace(/'/g, '"')) : testCase.expected;
        } catch (parseError) {
          // If parsing fails, try using the raw values
        ...
        const result = await Promise.race([
          new Promise(resolve => resolve(solutionFn(...inputArgs))),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Time Limit Exceeded')), 5000))
        ]);
        ...
        testResults.push({
          ...
          passed,
          explanation: passed ? 
            'Your solution produced the correct output.' : 
    ...
    const response = {
      ...
      memory: '14.2 MB', // This would require actual memory measurement
      feedback: allPassed ? 
        'Great job! Your solution passed all test cases.' :
        'Your solution failed some test cases. Please review the test results.'
    };
    ...
    res.status(500).json({ 
      status: 'Error',
      error: 'Failed to verify solution: ' + error.message,
      testResults: [],
      time: '0 ms',
      memory: '0 MB'
    });
...
// Get single problem (for admin panel)
